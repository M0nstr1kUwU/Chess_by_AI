        ...
        Row {
            for (file in 0..7) {
                ...
                val label = when (piece?.type) {
                    null -> " "
                    PieceType.Pawn -> if (piece.side == Side.White) "wP" else "bP"
                    PieceType.Knight -> if (piece.side == Side.White) "wN" else "bN"
                    PieceType.Bishop -> if (piece.side == Side.White) "wB" else "bB"
                    PieceType.Rook -> if (piece.side == Side.White) "wR" else "bR"
                    PieceType.Queen -> if (piece.side == Side.White) "wQ" else "bQ"
                    PieceType.King -> if (piece.side == Side.White) "wK" else "bK"
                }
...
private fun squareX(file: Int): Float = (7 - file) - 3.5f
private fun squareZ(rank: Int): Float = (7 - rank) - 3.5f
